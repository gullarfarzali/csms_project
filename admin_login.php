...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "SELECT password_hash FROM admin WHERE email = ? ";
    ...
    if ($result->num_rows === 1) {
        ...
        if ($hashedPassword === $_POST['password']){
            ...
            $stmt->close();
            $mysqli->close();
            header("Location: admin_dashboard.php"); // Redirect to the dashboard page or any other authenticated page
            exit();
        }
        else{
            // Login failed
            $stmt->close();
            $mysqli->close();
            header("Location: admin_login.php?msg=Invalid input"); // Redirect back to the login page with an error flag
            exit();
        }
    } else {
        // Login failed
